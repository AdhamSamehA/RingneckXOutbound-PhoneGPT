...
class ConversationalModel(Chain):
    ...
    def _prep_messages(self):
        ...
        """
        #self.determine_conversation_stage() # uncomment to use the stage analyzer chain
        prompt = self.conversation_chain.prep_prompts(
            [
                dict(
                    #conversation_stage=self.current_conversation_stage, # uncomment to use the stage analyzer chain
